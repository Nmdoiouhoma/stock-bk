...
#[AsCommand(name: 'app:seed', description: 'Seed the database with initial data (no-op if users already exist)')]
class SeedDatabaseCommand extends Command
{
    ...
    {
        $count = $this->em->getConnection()->executeQuery('SELECT COUNT(*) FROM "user"')->fetchOne();

        if ((int) $count > 0) {
            $output->writeln('Database already seeded, skipping.');
        ...
        $workstations = $this->loadWorkstations();
        $machines     = $this->loadMachines($workstations);
        $parts        = $this->loadParts($suppliers);
        $supervisor   = array_values(array_filter($users, fn(User $u) => $u->getRole() === Role::Supervisor))[0];
        $routings     = $this->loadRoutings($parts, $supervisor);
        $operations   = $this->loadOperations($routings, $workstations, $machines);
        $this->linkUserWorkstations($users, $workstations);
        $this->loadBoms($parts);
        $this->loadForecasts($operations);
        $this->loadCompletions($operations);
    ...
    /** @return Workstation[] */
    private function loadWorkstations(): array
    {
        $data = [
    ...
    /** @return Machine[] */
    private function loadMachines(array $workstations): array
    {
    ...
    /** @return Part[] */
    private function loadParts(array $suppliers): array
    {
    ...
    /** @return Routing[] */
    private function loadRoutings(array $parts, User $supervisor): array
    {
        [5 => $corps, 6 => $couvercle, 7 => $v100, 8 => $v50, 9 => $kit] = $parts;

        $data = [
            ['RT-001', 'Fabrication plateau de jeu',  $corps],
    ...
    /** @return Operation[] */
    private function loadOperations(array $routings, array $workstations, array $machines): array
    {
    ...
    private function linkUserWorkstations(array $users, array $workstations): void
    {
    ...
    private function loadBoms(array $parts): void
    {
    ...
    private function loadForecasts(array $operations): void
    {
        $data = [
    ...
    private function loadCompletions(array $operations): void
    {
        $data = [
            [0,  '-30 days', 50, 28.0],
            [1,  '-30 days', 50, 52.0],
