...
final class Version20260611123000 extends AbstractMigration
{
    ...
    {
        // Rename columns in operation table if they exist
        ...
        // Drop old FK referencing poste_de_travail_id (if present)
        $this->addSql('ALTER TABLE operation DROP CONSTRAINT IF EXISTS fk_1981a66d17a47ee6');
        $this->addSql('ALTER TABLE operation DROP CONSTRAINT IF EXISTS FK_1981A66D17A47EE6');

        // Add new FK for workstation_id -> workstation.id if missing
        $this->addSql("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_operation_workstation') THEN EXECUTE 'ALTER TABLE operation ADD CONSTRAINT fk_operation_workstation FOREIGN KEY (workstation_id) REFERENCES workstation (id) NOT DEFERRABLE'; END IF; END $$;");

        // Normalize column names for rank and unit_time: ensure they exist (no FK changes needed)
        // Remove duplicate supplier FK on part.supplier_id (keep fk_part_supplier)
        $this->addSql('ALTER TABLE part DROP CONSTRAINT IF EXISTS fk_44ca0b232add6d8c');
        // Ensure canonical fk_part_supplier exists
